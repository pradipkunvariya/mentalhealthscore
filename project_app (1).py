...
import os
if os.path.exists("student_mental_health_v3.csv"):
    raw_df, df = load_data("student_mental_health_v3.csv")
else:
    st.warning("⚠️ CSV file not found in the repository.")
    st.markdown(
        "Please upload **student_mental_health_v3.csv** below, "
        "or add it to your GitHub repo alongside project_app.py."
    )
    uploaded = st.file_uploader("Upload student_mental_health_v3.csv", type=["csv"])
    if uploaded is None:
        st.info(
            "👆 Upload the CSV file above to continue. "
            "To fix permanently: add student_mental_health_v3.csv to your GitHub repo."
        )
        st.stop()
